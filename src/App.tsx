...
function Counter({ to, suffix='' }: { to:number; suffix?:string }) {
  const ref = useRef<HTMLSpanElement>(null); const [val, setVal] = useState(0); const started = useRef(false)
  useEffect(() => {
    ...
    <header style={{ position:'fixed', inset:'0 0 auto', zIndex:50, transition:'all .3s', padding:scrolled?'0.5rem 0':'0.9rem 0' }} className={scrolled?'nav-scroll':''}>
      <div className="container" style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        ...
        <nav style={{ display:'flex', alignItems:'center', gap:'.1rem' }} className="desktop-nav">
          {NAV.map(l=>(
            <a key={l.href} href={l.href} style={{ padding:'.45rem .9rem', borderRadius:'.5rem', fontSize:'.85rem', color:'var(--text-2)', fontWeight:500, transition:'all .2s' }}
              onMouseEnter={e=>{(e.target as HTMLElement).style.background='var(--surface2)';(e.target as HTMLElement).style.color='var(--purple)'}}
              onMouseLeave={e=>{(e.target as HTMLElement).style.background='';(e.target as HTMLElement).style.color='var(--text-2)'}}
            >{l.label}</a>
          ))}
        </nav>
        <div style={{ display:'flex', alignItems:'center', gap:'.5rem' }}>
          ...
          <a href="#contact" className="btn btn-main" style={{ fontSize:'.82rem', padding:'.55rem 1.25rem' }}>Hire Me</a>
...
/* ── PROJECT CARD ── */
const AC: Record<string,{border:string;glow:string;tag:string;text:string;tagBg:string}> = {
...
function Splash({ onDone }: { onDone:()=>void }) {
  ...
  useEffect(()=>{
    const t1=setTimeout(()=>{ setPhase('load'); let p=0; const iv=setInterval(()=>{ p+=Math.random()*12+4; if(p>=100){p=100;clearInterval(iv)} setProg(Math.min(p,100)) },80) },300)
  ...
  return(
    <div style={{
      ...
      background:'linear-gradient(135deg, #0D0820 0%, #07050F 50%, #0A0618 100%)',
      ...
      <div style={{position:'absolute',inset:0,pointerEvents:'none'}}>
        {Array.from({length:14},(_,i)=>(
          <div key={i} style={{
            ...
            left:`${Math.random()*100}%`, top:`${Math.random()*100}%`,
            background:i%3===0?'rgba(124,58,237,0.6)':i%3===1?'rgba(8,145,178,0.6)':'rgba(217,119,6,0.5)',
            animation:`particle ${Math.random()*6+6}s ${Math.random()*8}s linear infinite`,
          }}/>
        ))}
      </div>

      {/* Orbs */}
      <div style={{position:'absolute',width:500,height:500,borderRadius:'50%',background:'rgba(124,58,237,0.15)',filter:'blur(100px)',top:'-20%',left:'-10%',pointerEvents:'none'}}/>
      <div style={{position:'absolute',width:400,height:400,borderRadius:'50%',background:'rgba(8,145,178,0.1)',filter:'blur(100px)',bottom:'-15%',right:'-5%',pointerEvents:'none'}}/>

      {/* LEFT — Photo (large) */}
      <div style={{ position:'relative', display:'flex', alignItems:'flex-end', justifyContent:'center', paddingBottom:0, overflow:'hidden' }}>
        {/* Gradient overlay on photo */}
        <div style={{ position:'absolute', inset:0, background:'linear-gradient(to right, transparent 60%, #07050F 100%)', zIndex:2, pointerEvents:'none' }}/>
        ...
        {/*
          ── YOUR PHOTO ──
          Upload your photo to /public/images/dharanidharan.jpg
          It will fill this entire left panel
        */}
        <div style={{
          position:'absolute', inset:0,
          background:'linear-gradient(145deg, rgba(124,58,237,0.2), rgba(8,145,178,0.1))',
          ...
          <img
            ...
            onError={e=>{
              ...
              (e.target as HTMLImageElement).parentElement!.innerHTML+=`<div style="display:flex;flex-direction:column;align-items:center;gap:1rem;color:rgba(255,255,255,0.4)"><span style="font-size:6rem">👤</span><span style="font-size:.75rem;font-family:monospace;letter-spacing:.1em">Upload dharanidharan.jpg to /public/images/</span></div>`
            }}
          />
        </div>

        {/* Name overlay on photo */}
        <div style={{ position:'relative', zIndex:3, padding:'2rem', alignSelf:'flex-end', width:'100%' }}>
          <h1 style={{ fontSize:'clamp(1.8rem,3.5vw,2.8rem)', fontWeight:900, color:'#fff', lineHeight:1.1, letterSpacing:'-0.03em', textShadow:'0 2px 20px rgba(0,0,0,0.5)' }}>
            Dharanidharan<br/><span style={{ background:'linear-gradient(135deg,#A78BFA,#22D3EE)', WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent' }}>R S</span>
          </h1>
        </div>
      </div>

      {/* RIGHT — Tagline & Info */}
      <div style={{ display:'flex', flexDirection:'column', justifyContent:'center', padding:'3rem 3rem 3rem 2rem', position:'relative', zIndex:3 }}>
        {/* Animated badge */}
        <div style={{ display:'inline-flex', alignItems:'center', gap:'.5rem', background:'rgba(124,58,237,0.15)', border:'1px solid rgba(124,58,237,0.4)', borderRadius:9999, padding:'.4rem 1rem', marginBottom:'2rem', width:'fit-content' }}>
          <span style={{ width:8, height:8, borderRadius:'50%', background:'#22D3EE', boxShadow:'0 0 10px #22D3EE', display:'inline-block', animation:'pulse 2s infinite' }}/>
          <span style={{ fontSize:'.68rem', fontFamily:'var(--ff-mono)', color:'#22D3EE', letterSpacing:'.2em' }}>PORTFOLIO 2026</span>
        </div>

        {/* Energetic Tagline */}
        <div style={{ marginBottom:'2rem' }}>
          <p style={{ fontSize:'clamp(1.1rem,2.5vw,1.5rem)', fontWeight:800, color:'#fff', lineHeight:1.4, letterSpacing:'-0.01em' }}>
            <span style={{ background:'linear-gradient(135deg,#A78BFA,#22D3EE,#FCD34D)', WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent', backgroundSize:'200%', animation:'shimmer 3s linear infinite' }}>
              Engineering the Future,
            </span>
            <br/>
            <span style={{ color:'rgba(255,255,255,0.9)' }}>One Line of Code</span>
            <br/>
            <span style={{ color:'rgba(255,255,255,0.9)' }}>at a Time. 🚀</span>
          </p>
        </div>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:'.6rem', marginBottom:'2.5rem' }}>
          {[
            {icon:'⚙️', label:'Mechanical Engineer', color:'rgba(124,58,237,0.2)', border:'rgba(124,58,237,0.5)', text:'#A78BFA'},
            {icon:'💻', label:'Full Stack Developer', color:'rgba(8,145,178,0.2)', border:'rgba(8,145,178,0.5)', text:'#22D3EE'},
            {icon:'✏️', label:'CAD Designer', color:'rgba(217,119,6,0.2)', border:'rgba(217,119,6,0.5)', text:'#FCD34D'},
          ].map(t=>(
            <div key={t.label} style={{ display:'flex', alignItems:'center', gap:'.6rem', background:t.color, border:`1px solid ${t.border}`, borderRadius:'.5rem', padding:'.5rem .85rem', width:'fit-content', animation:'slideIn .5s ease both' }}>
              <span style={{fontSize:'1rem'}}>{t.icon}</span>
              <span style={{ fontSize:'.8rem', fontFamily:'var(--ff-mono)', color:t.text, fontWeight:700, letterSpacing:'.08em' }}>{t.label}</span>
            </div>
          ))}
        </div>
        ...
        <div style={{ width:'100%', maxWidth:280 }}>
          <div style={{ display:'flex', justifyContent:'space-between', marginBottom:'.35rem' }}>
            <span style={{ fontSize:'.68rem', color:'rgba(255,255,255,.4)', fontFamily:'var(--ff-mono)' }}>Loading portfolio…</span>
            <span style={{ fontSize:'.68rem', color:'#22D3EE', fontFamily:'var(--ff-mono)', fontWeight:700 }}>{Math.round(prog)}%</span>
          </div>
          <div style={{ height:4, background:'rgba(255,255,255,.08)', borderRadius:999, overflow:'hidden' }}>
            <div style={{ height:'100%', width:`${prog}%`, background:'linear-gradient(90deg,#7C3AED,#0891B2,#D97706)', borderRadius:999, transition:'width .1s ease', boxShadow:'0 0 10px rgba(8,145,178,0.8)' }}/>
          </div>
        </div>
      </div>
      ...
      <style>{`
        @media(max-width:640px){
          div[style*="gridTemplateColumns"]{grid-template-columns:1fr!important;grid-template-rows:1fr auto}
        }
      `}</style>
    </div>
  )
...
const ACHV_C: Record<string,{bg:string;border:string;text:string;glow:string}> = {
    ...
    <>
      ...
      <div style={{minHeight:'100vh',overflowX:'hidden',background:'var(--bg)'}}>
        ...
        <section style={{position:'relative',minHeight:'100vh',display:'flex',alignItems:'center',paddingTop:100,paddingBottom:80,overflow:'hidden'}}>
          ...
          <Particles/>
          <div className="orb" style={{width:700,height:700,background:dark?'rgba(124,58,237,0.1)':'rgba(124,58,237,0.07)',top:'-25%',left:'-15%'}}/>
          <div className="orb" style={{width:500,height:500,background:dark?'rgba(8,145,178,0.08)':'rgba(8,145,178,0.05)',bottom:'-15%',right:'-10%'}}/>
              ...
              <Reveal delay={80}>
                <div className="card" style={{padding:'2rem',display:'flex',alignItems:'flex-start',gap:'1rem'}}>
                  ...
                  <div>
                    ...
                    <p style={{fontSize:'.92rem',color:'var(--text-2)',lineHeight:1.7}}>Hands-on exposure to industrial manufacturing workflows, mechanical systems and quality practices.</p>
                  </div>
                </div>
              </Reveal>
            </div>
        ...
        <section id="mechanical" className="section">
          ...
          <div className="orb" style={{width:600,height:600,background:dark?'rgba(124,58,237,0.07)':'rgba(124,58,237,0.04)',top:'5%',left:'-15%'}}/>
              ...
              <div style={{display:'flex',gap:'.75rem',marginBottom:'2rem',flexWrap:'wrap'}}>
                {([['all','All Projects'],['hardware','🔧 Hardware & Innovation'],['cad','💻 Advanced CAD']] as const).map(([k,label])=>(
                  <button key={k} onClick={()=>setMechF(k)} style={{padding:'.55rem 1.25rem',borderRadius:9999,border:'1.5px solid',
                    ...
                    cursor:'pointer',fontSize:'.84rem',fontWeight:600,fontFamily:'var(--ff-display)',transition:'all .25s',
                    boxShadow:mechF===k?'var(--shadow-sm)':'none'}}>{label}</button>
                ))}
              </div>
            </Reveal>
        ...
        <section id="software" className="section" style={{background:'var(--bg2)'}}>
          <div className="divider" style={{position:'absolute',top:0,left:0,right:0}}/>
          <div className="orb" style={{width:600,height:600,background:dark?'rgba(8,145,178,0.07)':'rgba(8,145,178,0.04)',top:'5%',right:'-15%'}}/>
          <div className="container" style={{position:'relative',zIndex:1}}>
            <Reveal>
              <div style={{display:'flex',alignItems:'center',gap:'.75rem',marginBottom:'1rem'}}>
              ...
              {EDU.map((e,i)=>(
                <Reveal key={e.school+i} delay={i*80}>
                  <div className="card" style={{padding:'2rem',height:'100%'}}
                    onMouseEnter={el=>{(el.currentTarget as HTMLElement).style.borderColor='var(--border-h)'}}
                ...
                return(
                  <Reveal key={a.title} delay={i*60}>
                    <div className="card" style={{padding:'1.75rem',height:'100%',position:'relative',overflow:'hidden'}}
                      onMouseEnter={el=>{const e=el.currentTarget as HTMLElement;e.style.borderColor=c.border;e.style.boxShadow=`0 12px 40px ${c.glow}, var(--shadow-lg)`}}
                      onMouseLeave={el=>{const e=el.currentTarget as HTMLElement;e.style.borderColor='var(--border)';e.style.boxShadow='var(--shadow-sm)'}}
                    >
                      <div style={{position:'absolute',top:-20,right:-20,width:80,height:80,borderRadius:'50%',background:c.bg,filter:'blur(20px)'}}/>
                      <span style={{width:50,height:50,borderRadius:'.875rem',background:c.bg,border:`1px solid ${c.border}`,display:'grid',placeItems:'center',color:c.text,marginBottom:'1rem',position:'relative',boxShadow:`0 4px 16px ${c.glow}`}}>
                        <Icon size={22}/>
                      </span>
                ...
                <div className="card" style={{padding:'2rem'}}>
                  ...
                  <div style={{display:'flex',flexDirection:'column',gap:'.75rem'}}>
                    {[
                      ...
                      <a key={l.label} href={l.href} target="_blank" rel="noreferrer" style={{display:'flex',alignItems:'center',gap:'.875rem',padding:'.75rem 1rem',borderRadius:'.75rem',border:'1px solid var(--border)',background:'var(--surface2)',transition:'all .25s',textDecoration:'none'}}
                        onMouseEnter={e=>{const el=e.currentTarget as HTMLElement;el.style.borderColor='var(--border-h)';el.style.boxShadow='var(--shadow-md)'}}
                        onMouseLeave={e=>{const el=e.currentTarget as HTMLElement;el.style.borderColor='var(--border)';el.style.boxShadow='none'}}
                      >
                        <span style={{width:42,height:42,borderRadius:'.625rem',background:'var(--surface)',border:'1px solid var(--border)',display:'grid',placeItems:'center',color:l.color,flexShrink:0,boxShadow:'var(--shadow-sm)'}}><l.icon size={17}/></span>
                        <div>
                          <div style={{fontSize:'.68rem',color:'var(--text-3)',fontFamily:'var(--ff-mono)',fontWeight:600}}>{l.label}</div>
                          <div style={{fontSize:'.88rem',fontFamily:'var(--ff-mono)',color:'var(--text)',fontWeight:600}}>{l.value}</div>
                        </div>
                      </a>
                    ))}
                  </div>
                  ...
                  <div style={{marginTop:'1.25rem',display:'flex',alignItems:'center',gap:'.5rem',fontSize:'.82rem',color:'var(--text-3)'}}>
          ...
          <div className="container" style={{padding:'2.5rem 1.25rem',display:'flex',flexWrap:'wrap',alignItems:'center',justifyContent:'space-between',gap:'1.5rem'}}>
            ...
            <div style={{display:'flex',alignItems:'center',gap:'.625rem'}}>
              {[{href:'mailto:s2007dharanidharan@gmail.com',icon:Mail,label:'Email'},{href:'https://github.com/s2007dharanidharan-cloud',icon:Github,label:'GitHub'},{href:'https://www.linkedin.com/in/dharanidharan-R-S',icon:Linkedin,label:'LinkedIn'}].map(l=>(
                <a key={l.label} href={l.href} target="_blank" rel="noreferrer" aria-label={l.label} style={{width:42,height:42,borderRadius:'50%',border:'1px solid var(--border)',background:'var(--surface2)',display:'grid',placeItems:'center',color:'var(--text-2)',transition:'all .25s',textDecoration:'none'}}
                  onMouseEnter={e=>{(e.currentTarget as HTMLElement).style.borderColor='var(--border-h)';(e.currentTarget as HTMLElement).style.color='var(--purple)';(e.currentTarget as HTMLElement).style.boxShadow='var(--shadow-md)'}}
                  onMouseLeave={e=>{(e.currentTarget as HTMLElement).style.borderColor='var(--border)';(e.currentTarget as HTMLElement).style.color='var(--text-2)';(e.currentTarget as HTMLElement).style.boxShadow='none'}}
                ><l.icon size={17}/></a>
              ))}
