/* ============================================================
   Mild Orange Splash Cursor
   Vanilla-JS port of React Bits' SplashCursor (WebGL fluid sim)
   Fixed warm-orange palette, no rainbow, tuned to be subtle.
   ============================================================ */
(function () {
  ...
  const displayShaderSource = `
    ...
    void main () {
      ...
      // while the cursor is nearly still) can't accumulate into a glow
      // brighter than intended, regardless of how many splats land nearby.
      c = min(c, vec3(0.55));
  ...
  function hexToRGB(hex) {
    ...
    return { r: r * config.INTENSITY, g: g * config.INTENSITY, b: b * config.INTENSITY };
  }

  // Fixed mild-orange palette generator — replaces RAINBOW_MODE's HSV cycling.
  // Picks a random shade from PALETTE (or uses COLOR if PALETTE has 1 entry).
  function generateColor() {
    if (config.RAINBOW_MODE) {
      let c = HSVtoRGB(Math.random(), 1.0, 1.0);
      c.r *= config.INTENSITY; c.g *= config.INTENSITY; c.b *= config.INTENSITY;
      return c;
    }
    const palette = config.PALETTE && config.PALETTE.length ? config.PALETTE : [config.COLOR];
  ...
  // Cleanup hook in case this script is ever torn down (SPA navigation, etc.)
  window.__splashCursorCleanup = function () {
    isActive = false;
